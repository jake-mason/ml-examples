...
system('virtualenv ~/.virtualenvs/venv')
...
# Attempt #1
transformers <- list(
	c('mpg', sk_preprocessing$StandardScaler(), tuple('mpg')),
	c('wt', 'passthrough', tuple('wt')),
	c('gear', sk_preprocessing$OneHotEncoder(), tuple('gear'))
	# TODO: handle SimpleImputer, too?
)

column_transformer <- sk_compose$ColumnTransformer(transformers)
